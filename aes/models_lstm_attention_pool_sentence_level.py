...
def main():
    ...
    # att1_output = tf.reshape(att1_output, [-1, hp.e_len, 1, hp.lstm_sen_level_convunits_size])
    ...
    lstm_output, _ = tf.nn.dynamic_rnn(lstm_cell, att1_output, dtype=tf.float32)

    # attention pooling
    att2_mat = tf.Variable(tf.truncated_normal([hp.lstm_sen_level_att_pool_hidden_size, hp.lstm_sen_level_att_pool_hidden_size]), dtype=tf.float32)
    att2_bias = tf.Variable(tf.truncated_normal([1, 1, hp.lstm_sen_level_att_pool_hidden_size]), dtype=tf.float32)
    att2_weight = tf.tensordot(lstm_output, att2_mat, axes=[2, 0])
    att2_weight = tf.nn.tanh(att2_weight)
    att2_vec = tf.Variable(tf.truncated_normal([hp.lstm_sen_level_att_pool_hidden_size, 1]), dtype=tf.float32)
